...
# 57 "./mcc_generated_files/eccp2.h"
# 1 "./mcc_generated_files/../main.h" 1
# 25 "./mcc_generated_files/../main.h"
...
uint16_t bitCounts[33*2];
uint16_t bitDurations[33*2];

uint32_t numberButtonCodes[4];
uint32_t buttonSequenceCodes[8];

uint16_t tmrCounts;

uint8_t transmitting = 0;
uint8_t transmittingFinished = 0;
...
void recordDurations(_Bool verbose);
void setSevenDurations(void);
uint32_t decodeBits(_Bool printBinary);
...
# 57 "./mcc_generated_files/eccp2.h" 2
# 81 "./mcc_generated_files/eccp2.h"
...
# 127 "./mcc_generated_files/eccp2.h"
void ECCP2_Initialize(void);
# 154 "./mcc_generated_files/eccp2.h"
void ECCP2_SetCompareCount(uint16_t compareCount);
# 170 "./mcc_generated_files/eccp2.h"
...
# 123 "./mcc_generated_files/eccp1.h"
void ECCP1_Initialize(void);
# 150 "./mcc_generated_files/eccp1.h"
_Bool ECCP1_IsCapturedDataReady(void);
# 180 "./mcc_generated_files/eccp1.h"
...
void recordDurations(_Bool verbose) {
    ...
    for(i = 0; i < 33*2 - 1; i++) {
        bitDurations[i] = (bitCounts[i+1] - bitCounts[i]) * (8)/(16);
        if(verbose)
            printf("%u us\r\n", bitDurations[i]);
    }
}
...
uint32_t decodeBits(_Bool printBinary) {
    ...
    uint8_t binaryDigit;
    for(uint8_t i = 2; i < (33*2)-1; i+=2) {
        durationLow = bitDurations[i];
        durationHigh = bitDurations[i+1];
        if(((durationLow > referenceDurations.dataZeroLow-(referenceDurations.dataZeroLow*0.1f)) && (durationLow < referenceDurations.dataZeroLow+(referenceDurations.dataZeroLow*0.1f)))
           &&( (durationHigh > referenceDurations.dataZeroHigh-(referenceDurations.dataZeroHigh*0.1f)) && (durationHigh < referenceDurations.dataZeroHigh+(referenceDurations.dataZeroHigh*0.1f)) )) {
                decodedVal = decodedVal<<1;
                if(printBinary)
                    printf("0");
        }

        else if(( (durationLow > referenceDurations.dataOneLow-(referenceDurations.dataOneLow*0.1f)) && (durationLow < referenceDurations.dataOneLow+(referenceDurations.dataOneLow*0.1f)) )
            && ((durationHigh > referenceDurations.dataOneHigh-(referenceDurations.dataOneHigh*0.1f)) && (durationHigh < referenceDurations.dataOneHigh+(referenceDurations.dataOneHigh*0.1f)))) {
    ...
    if(transmitting) {
        switch(state) {
            ...
            case TX_STOP:
                ...
                state = TX_IDLE;
                transmitting = 0;
                transmittingFinished = 1;
                break;
        }
    }
    CCPR2 = TMR1+ waitDuration/(8)/(16);
